...
const animatePrompts = async (): Promise<void> => {
...
const animatePasswordGeneration = async (): Promise<void> => {
  const generationAnimation = chalkAnimation.karaoke(
...
const generatePassword = (options: string[], passLength: number): string => {
...
const generatedPassword: string = generatePassword(
  ...
  answers.passwordLength
);

console.log(
  `${chalk.yellow("Your generated password is:")} ${chalk
    .hex("#6495ED")
...
const passwordStrength: string = checkPasswordStrength(generatedPassword);
switch (passwordStrength) {
  case "Weak":
    console.log(
      chalk.red(`Password Strength: ${chalk.italic(passwordStrength)}`)
    );
    break;
  case "Moderate":
    console.log(
      chalk.hex("#FF9800")(
        `Password Strength: ${chalk.italic(passwordStrength)}`
      )
    );
    break;
  case "Strong":
    console.log(
      chalk.green(`Password Strength: ${chalk.italic(passwordStrength)}`)
    );
    break;
  default:
